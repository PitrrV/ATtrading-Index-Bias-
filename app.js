...
function applyLiveData(futures, quotes, calendar) {
...
async function refreshLiveData() {
  ...
  // 1. Futures (Yahoo Finance) — s fallback na demo ceny
  let futures = [];
  ...
  try {
    ...
    // Použít stávající ceny z modelu jako fallback
  ...
  applyLiveData(futures, quotes, calendar);
...
function renderBreakdown() {
  const rows = [
    ['Premarket',  selected.premarketScore,   '40%', selected.premarketScore   * 0.40],
    ['Momentum',   selected.momentumScore,    '20%', selected.momentumScore    * 0.20],
    ['Top Stocks', selected.topStockScore,    '15%', selected.topStockScore    * 0.15],
    ['Fundament',  selected.fundamentalScore, '20%', selected.fundamentalScore * 0.20],
    ['Sector',     selected.sectorScore,      '5%',  selected.sectorScore      * 0.05]
  ];
  document.getElementById('breakdown').innerHTML =
    ...
    rows.map(([name,raw,weight,impact]) => {
      ...
      return `<div class="break-row"><b>${name}</b><span class="${cls}">${raw.toFixed(2)}</span><span>${weight}</span><b class="${cls}">${impact>0?'+':''}${impact.toFixed(2)}</b></div>`;
    }).join('');
}
...
function renderStocks() {
  ...
  document.getElementById('stocksTable').innerHTML =
    `<div class="stock-row head"><span>Ticker</span><span>Váha</span><span>14:00</span><span>Now</span><span>Impact</span></div>` +
    sorted.map(s => {
      ...
      return `<div class="stock-row"><b>${s.ticker}</b><span>${s.weight}%</span><span>${signed(s.premarketAt14)}</span><span class="${s.premarketNow>=0?'bull':'bear'}">${signed(s.premarketNow)}</span><div><b class="${cls}">${impact>0?'+':''}${impact.toFixed(3)}</b><div class="impact-bar"><span style="width:${barWidth}%;background:${impact>=0?'var(--green)':'var(--red)'}"></span></div><small>${fade<-0.3?'📉 fade':fade>0.3?'📈 strength':'stable'}</small></div></div>`;
    }).join('');
}
