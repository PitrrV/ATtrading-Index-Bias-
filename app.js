...
const demoData = {
  ...
  _demoMacro: { yieldsChangePct: -0.6, dxyChangePct: -0.2, vix: 14.2, vix3m: 16.1, smallCapChangePct: 0.74, allAligned: true },
  indices: [
    {
      id: 'US100', name: 'NASDAQ-100',
      futuresSymbol: 'NQ=F',
      prevClose: 21200.00, premarketPrice: 21350.00,
      atr5d: 285, prevSession: { high: 21480, low: 21060, close: 21380, closePosition: 0.76, pdh: 21480, pdl: 21060, pdc: 21380 },
      levels: { premarketHigh: 21410, premarketLow: 21280, vwap: 21355, poc: 21340, vah: 21395, val: 21300 },
      vixChangePct: null,
      stocks: [
    ...
    },
    {
      ...
      atr5d: 72, prevSession: { high: 5840, low: 5762, close: 5820, closePosition: 0.74, pdh: 5840, pdl: 5762, pdc: 5820 },
      levels: { premarketHigh: 5835, premarketLow: 5805, vwap: 5822, poc: 5818, vah: 5832, val: 5808 },
      vixChangePct: null,
      stocks: [
    ...
    },
    {
      ...
      atr5d: 430, prevSession: { high: 41720, low: 41280, close: 41650, closePosition: 0.83, pdh: 41720, pdl: 41280, pdc: 41650 },
      levels: { premarketHigh: 41600, premarketLow: 41510, vwap: 41560, poc: 41555, vah: 41595, val: 41520 },
      vixChangePct: null,
      stocks: [
...
// Macro symboly (Yahoo Finance, zdarma) — výnosy, VIX term, dolar, small caps
const MACRO_SYMBOLS = {
  yield: '^TNX',
  vix:   '^VIX',
  vix3m: '^VIX3M',
...
const storage = {
  ...
  get dayCache()  { try { return JSON.parse(localStorage.getItem('AT_DAY_CACHE') || '{}'); } catch { return {}; } },
  set dayCache(v) { localStorage.setItem('AT_DAY_CACHE', JSON.stringify(v)); },
  get calCache()  { try { return JSON.parse(localStorage.getItem('AT_CAL_CACHE') || '{}'); } catch { return {}; } },
  set calCache(v) { localStorage.setItem('AT_CAL_CACHE', JSON.stringify(v)); }
};
...
// ─── Fetch: Yahoo Finance Futures + Intradenní úrovně + VWAP ─────────────────
// Ze STEJNÝCH 1m dat vytáhneme navíc Premarket H/L a VWAP — bez extra API volání.
async function fetchYahooFutures() {
  const symbols = Object.keys(FUTURES_MAP);
  const results = [];
  for (const sym of symbols) {
    const encoded = sym.replace('=', '%3D');
    ...
    // Intradenní úrovně + VWAP z 1m svíček
    const ts = res.timestamp || [];
    const q  = res.indicators?.quote?.[0] || {};
    const etFmt = new Intl.DateTimeFormat('en-US', {
      timeZone: 'America/New_York', hour: '2-digit', minute: '2-digit', hour12: false
    });
    let pmHigh = -Infinity, pmLow = Infinity, cumPV = 0, cumV = 0;
    const priceVols = [];
    for (let i = 0; i < ts.length; i++) {
      const h = q.high?.[i], l = q.low?.[i], c = q.close?.[i], v = q.volume?.[i] || 0;
      if (h == null || l == null || c == null) continue;
      const parts = etFmt.formatToParts(new Date(ts[i] * 1000));
      const etMin = parseInt(parts.find(p => p.type === 'hour').value) * 60
                  + parseInt(parts.find(p => p.type === 'minute').value);
      cumPV += ((h + l + c) / 3) * v; cumV += v;
      if (v > 0) priceVols.push({ price: c, vol: v });
    ...
    results.push({
      symbol: sym, indexId: FUTURES_MAP[sym], price, prevClose,
      changePct: ((price - prevClose) / prevClose) * 100,
      levels: {
        premarketHigh: pmHigh > -Infinity ? pmHigh : null,
        premarketLow:  pmLow  <  Infinity ? pmLow  : null,
        vwap:          cumV > 0 ? cumPV / cumV : null,
        ...buildVolProfile(priceVols)
      }
    });
  }
  return results;
...
// Volume Profile — POC + Value Area High/Low z 1m svíček
function buildVolProfile(priceVols) {
  if (!priceVols || priceVols.length < 5) return {};
  const prices = priceVols.map(p => p.price);
  const min = Math.min(...prices), max = Math.max(...prices), range = max - min;
  if (range <= 0) return {};
  const bins = 50, binSize = range / bins;
  const hist = new Array(bins).fill(0);
  priceVols.forEach(({ price, vol }) => {
    let i = Math.floor((price - min) / binSize);
    if (i >= bins) i = bins - 1; if (i < 0) i = 0;
    hist[i] += vol;
  });
  let pocIdx = 0, maxV = 0;
  hist.forEach((v, i) => { if (v > maxV) { maxV = v; pocIdx = i; } });
  const poc = min + (pocIdx + 0.5) * binSize;
  const total = hist.reduce((a, b) => a + b, 0);
  let va = hist[pocIdx], lo = pocIdx, hi = pocIdx;
  while (va < total * 0.70 && (lo > 0 || hi < bins - 1)) {
    const below = lo > 0 ? hist[lo - 1] : -1;
    const above = hi < bins - 1 ? hist[hi + 1] : -1;
    if (above >= below) { hi++; va += hist[hi]; } else { lo--; va += hist[lo]; }
  }
  return { poc, vah: min + (hi + 1) * binSize, val: min + lo * binSize };
}

// Macro quote (Yahoo Finance) — výnosy, VIX, DXY, IWM
async function fetchYahooQuote(symbol) {
  const encoded = symbol.replace('=', '%3D').replace('^', '%5E');
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encoded}?interval=1d&range=5d`;
  const r = await fetch(url, { headers: { 'Accept': 'application/json' } });
  if (!r.ok) throw new Error(`Yahoo quote ${r.status} pro ${symbol}`);
  const d = await r.json();
  const meta = d.chart?.result?.[0]?.meta;
  if (!meta) return null;
  const price = meta.regularMarketPrice;
  const prevClose = meta.chartPreviousClose || meta.previousClose;
  return { symbol, price, prevClose, changePct: prevClose ? ((price - prevClose) / prevClose) * 100 : null };
}
...
// Vrací i PDH/PDL/PDC (previous day high/low/close) pro intradenní úrovně
function getPrevSession(candles) {
  ...
  return {
    high: prev.high, low: prev.low, close: prev.close, closePosition,
...
function applyLiveData(futures, quotes, calendar, historyMap = {}, macroData = {}) {
  ...
  // ── Macro vrstva ──────────────────────────────────────────────────────────
  const yieldsChangePct = macroData.yield?.changePct ?? null;
  const dxyChangePct    = macroData.dxy?.changePct ?? null;
  const vixLevel        = macroData.vix?.price ?? null;
  const vix3mLevel      = macroData.vix3m?.price ?? null;
  const smallCapChange  = macroData.iwm?.changePct ?? null;
  const nqChange = futures.find(f => f.indexId === 'US100')?.changePct ?? null;
  const esChange = futures.find(f => f.indexId === 'US500')?.changePct ?? null;
  const ymChange = futures.find(f => f.indexId === 'US30')?.changePct ?? null;
  const allCh = [nqChange, esChange, ymChange].filter(x => x != null);
  const allAligned = allCh.length >= 2 && (allCh.every(c => c > 0.1) || allCh.every(c => c < -0.1));
  ...
  demoData.indices.forEach(index => {
    ...
    if (fut) {
      ...
      if (fut.levels) index.levels = fut.levels; // premarket H/L + VWAP
    }
    ...
    // Macro vrstva — pole pro V6 engine
    index.yieldsChangePct   = yieldsChangePct;
    index.dxyChangePct      = dxyChangePct;
    index.vix               = vixLevel;
    index.vix3m             = vix3mLevel;
    index.smallCapChangePct = smallCapChange;
    index.allAligned        = allAligned;
...
async function refreshLiveData() {
  ...
  try {
    ...
    // ── CACHE: historická data (ATR, prevSession, PDH/PDL, avgVol) se mění 1×/den ──
    const dc = storage.dayCache;
    const cacheValid = dc.date === todayKey() && dc.historyMap;
    if (cacheValid) {
      Object.assign(historyMap, dc.historyMap);
      setApiStatus('Historická data z cache (rychlé)…', true);
    } else {
      setApiStatus('Načítám historii (1× denně)…', true);
      ...
      // Ulož do cache (bez _volumeMap, ten je živý)
      const toCache = {};
      Object.keys(historyMap).forEach(k => { if (k !== '_volumeMap') toCache[k] = historyMap[k]; });
      storage.dayCache = { date: todayKey(), historyMap: toCache };
    }

    // Premarket volume — vždy živé (mění se intraday), ale jen v premarket oknu
    const isPremarketWindow = czHour() >= 10 && czHour() < 16;
    if (isPremarketWindow) {
      setApiStatus('Načítám premarket volume…', true);
      const volResults = await Promise.allSettled(
        VOLUME_SYMBOLS.map(async sym => ({ symbol: sym, pmVol: await fetchPremarketVolume(sym) }))
      );
      const volumeMap = {};
      volResults.forEach(r => {
        if (r.status === 'fulfilled' && r.value?.pmVol) volumeMap[r.value.symbol] = r.value.pmVol;
      });
  ...
  // 4. Macro vrstva (Yahoo Finance) — výnosy, VIX term, dolar, small caps
  setApiStatus('Načítám macro (výnosy, VIX, DXY, small caps)…', true);
  const macroData = {};
  try {
    const mr = await Promise.allSettled(
      Object.entries(MACRO_SYMBOLS).map(async ([k, sym]) => ({ k, q: await fetchYahooQuote(sym) }))
    );
    mr.forEach(r => { if (r.status === 'fulfilled' && r.value.q) macroData[r.value.k] = r.value.q; });
  } catch(e) { console.warn('Macro fetch:', e.message); }

  if (!quotes.length) throw new Error('Nepodařilo se načíst žádné quotes. Zkontroluj API key.');

  applyLiveData(futures, quotes, calendar, historyMap, macroData);
...
// ─── State ────────────────────────────────────────────────────────────────────
// Attach demo macro data to indices for initial render (demo mode)
demoData.indices.forEach(idx => {
  const m = demoData._demoMacro;
  idx.yieldsChangePct   = m.yieldsChangePct;
  idx.dxyChangePct      = m.dxyChangePct;
  idx.vix               = m.vix;
  idx.vix3m             = m.vix3m;
  idx.smallCapChangePct = m.smallCapChangePct;
  idx.allAligned        = m.allAligned;
});
...
function renderPrimary() {
  ...
  const priceLabel = selected.dataSource === 'LIVE_FUTURES'
    ? `${selected.futuresSymbol} futures (přesná cena)`
    : 'demo cena';
  const strVal = selected.strength?.value ?? selected.confidence ?? 0;
  const strLbl = selected.strength?.label ?? '';
  const strCls = selected.strength?.cls ?? cls;
  const gapR   = selected.gapRead || {};
  const ratioStr = gapR.ratio != null ? gapR.ratio.toFixed(2) + '×' : '—';
  const vt = selected.vixTerm || {};
  const vc = window.IndexBiasEngine?.volumeConvictionLabel(selected.volumeRatio);
  document.getElementById('primaryDecision').innerHTML = `
    <article class="d-card highlight">
      <div class="d-label">Primary Decision</div>
      <div class="d-val ${cls}">${selected.id}: ${selected.bias}</div>
      <div class="d-sub">Cena: <b>${fmt(selected.premarketPrice)}</b> · ${priceLabel}</div>
    </article>
    <article class="d-card">
      <div class="d-label">Signal Strength</div>
      <div class="d-val ${strCls}">${strVal}% <span style="font-size:12px">${strLbl}</span></div>
      <div class="d-sub">Score: ${selected.totalScore.toFixed(2)} / ±2${selected.freeze?.frozen ? ' · <b class="neutral">⏳ Zmražen</b>' : ''}</div>
    </article>
    <article class="d-card">
      <div class="d-label">Scénář</div>
      <div class="d-val ${selected.scenario.mode}" style="font-size:16px;line-height:1.25">${selected.scenario.name}</div>
      <div class="d-sub">Pravděpodobnost: ${selected.scenario.probability}%</div>
    </article>
    <article class="d-card">
      <div class="d-label">Momentum od 14:00</div>
      <div class="d-val ${selected.momentumRaw >= 0 ? 'bull' : 'bear'}">${signed(selected.momentumRaw)}</div>
      <div class="d-sub">${deltaRead}</div>
    </article>
    <article class="d-card">
      <div class="d-label">Gap / ATR (5d)</div>
      <div class="d-val ${gapR.cls||'neutral'}" style="font-size:15px;line-height:1.25">${gapR.label||'—'}</div>
      <div class="d-sub">Gap <b>${signed(selected.gapPct||0)}</b> · ratio <b>${ratioStr}</b></div>
    </article>
    <article class="d-card">
      <div class="d-label">Macro — 10Y + Dolar</div>
      <div class="d-val ${selected.macroRead?.cls||'neutral'}" style="font-size:15px;line-height:1.25">${selected.macroRead?.label||'—'}</div>
      <div class="d-sub">10Y: <b>${selected.yieldsChangePct!=null?signed(selected.yieldsChangePct):'—'}</b> · DXY: <b>${selected.dxyChangePct!=null?signed(selected.dxyChangePct):'—'}</b></div>
    </article>
    <article class="d-card">
      <div class="d-label">Breadth — NQ/ES/IWM</div>
      <div class="d-val ${selected.breadthRead?.cls||'neutral'}" style="font-size:14px;line-height:1.25">${selected.breadthRead?.label||'—'}</div>
      <div class="d-sub">Small caps (IWM): <b>${selected.smallCapChangePct!=null?signed(selected.smallCapChangePct):'—'}</b></div>
    </article>
    <article class="d-card">
      <div class="d-label">VIX Term Structure</div>
      <div class="d-val ${vt.cls||'neutral'}" style="font-size:14px;line-height:1.25">${vt.label||'—'}</div>
      <div class="d-sub">${vt.ratio!=null?'VIX/VIX3M: '+vt.ratio.toFixed(3):'data se načítají'}</div>
    </article>
    <article class="d-card">
      <div class="d-label">Volume Conviction</div>
      <div class="d-val ${vc?.cls||'neutral'}" style="font-size:15px;line-height:1.25">${vc?.icon||''} ${vc?.label||'—'}</div>
      <div class="d-sub">Avg top5: <b>${selected.volumeRatio!=null?selected.volumeRatio.toFixed(2)+'×':'—'}</b></div>
    </article>
    <article class="d-card">
      <div class="d-label">Seasonality</div>
      <div class="d-val neutral" style="font-size:13px;line-height:1.3">${selected.seasonRead||'—'}</div>
      <div class="d-sub">Kalendářní efekty</div>
    </article>`;
}
...
function renderBreakdown() {
  const rows = [
    ['Premarket',       selected.premarketScore,    '22%', selected.premarketScore    * 0.22],
    ['Fundament',       selected.fundamentalScore,  '18%', selected.fundamentalScore  * 0.18],
    ['Macro 10Y+DXY',   selected.macroScore,        '12%', selected.macroScore        * 0.12],
    ['Gap/ATR',         selected.gapContextScore,   '12%', selected.gapContextScore   * 0.12],
    ['Momentum',        selected.momentumScore,     '11%', selected.momentumScore     * 0.11],
    ['Breadth RS',      selected.breadthScore,      '9%',  selected.breadthScore      * 0.09],
    ['Top Stocks',      selected.topStockScore,     '6%',  selected.topStockScore     * 0.06],
    ['Prev Session',    selected.prevSessionScore,  '6%',  selected.prevSessionScore  * 0.06],
    ['Seasonality',     selected.seasonalityScore,  '4%',  selected.seasonalityScore  * 0.04]
  ];
...
// ─── renderAll ────────────────────────────────────────────────────────────────
// ─── Render: Intradenní úrovně (PDH/PDL/PDC + Premarket H/L + VWAP) ──────────
function renderLevels() {
  const el = document.getElementById('levelsPanel');
  if (!el) return;
  const px = selected.premarketPrice;
  const ps = selected.prevSession || {};
  const lv = selected.levels || {};
  const rows = [
    { label: 'PDH',  desc: 'Previous Day High',  val: ps.pdh,  type: 'res' },
    { label: 'PM-H', desc: 'Premarket High',      val: lv.premarketHigh, type: 'res' },
    { label: 'VAH',  desc: 'Value Area High',      val: lv.vah,  type: 'res' },
    { label: 'VWAP', desc: 'Session VWAP',         val: lv.vwap, type: 'vwap' },
    { label: 'POC',  desc: 'Point of Control (magnet)', val: lv.poc, type: 'vwap' },
    { label: 'PDC',  desc: 'Previous Day Close',   val: ps.pdc,  type: 'mid' },
    { label: 'VAL',  desc: 'Value Area Low',        val: lv.val,  type: 'sup' },
    { label: 'PM-L', desc: 'Premarket Low',        val: lv.premarketLow, type: 'sup' },
    { label: 'PDL',  desc: 'Previous Day Low',     val: ps.pdl,  type: 'sup' }
  ].filter(r => r.val != null)
   .map(r => ({ ...r, dist: ((px - r.val) / r.val) * 100 }))
   .sort((a, b) => b.val - a.val);

  // Najdi nejbližší rezistenci nad cenou a support pod cenou
  const above = rows.filter(r => r.val > px).sort((a,b)=>a.val-b.val)[0];
  const below = rows.filter(r => r.val < px).sort((a,b)=>b.val-a.val)[0];

  el.innerHTML = `
    <div class="panel-head">
      <div><h3>Intradenní úrovně — ${selected.id}</h3><p>Kde jednat · vzdálenost od aktuální ceny ${fmt(px)}</p></div>
      <span class="pill green-pill">PDH/PDL · PM · VWAP</span>
    </div>
    <div class="levels-now">
      <span class="mini">Aktuální cena</span>
      <b class="${selected.bias.includes('LONG')?'bull':selected.bias.includes('SHORT')?'bear':'neutral'}">${fmt(px)}</b>
    </div>
    <div class="levels-list">
      ${rows.map(r => {
        const isNear = (above && r.label===above.label) || (below && r.label===below.label);
        const cls = r.type==='res'?'lvl-res':r.type==='sup'?'lvl-sup':r.type==='vwap'?'lvl-vwap':'lvl-mid';
        const side = r.val > px ? 'nad' : 'pod';
        return `<div class="lvl-row ${cls} ${isNear?'lvl-near':''}">
          <span class="lvl-tag">${r.label}</span>
          <span class="lvl-desc">${r.desc}${isNear?' <b class="bull">◄ nejbližší</b>':''}</span>
          <span class="lvl-val">${fmt(r.val)}</span>
          <span class="lvl-dist ${r.dist>=0?'bull':'bear'}">${r.dist>=0?'+':''}${r.dist.toFixed(2)}% ${side}</span>
        </div>`;
      }).join('')}
    </div>
    ${above || below ? `<div class="levels-read">
      ${above ? `<div><span class="mini">Nejbližší odpor</span><br><b class="bear">${above.label} ${fmt(above.val)}</b> <small>(+${Math.abs(above.dist).toFixed(2)}%)</small></div>` : ''}
      ${below ? `<div><span class="mini">Nejbližší support</span><br><b class="bull">${below.label} ${fmt(below.val)}</b> <small>(-${Math.abs(below.dist).toFixed(2)}%)</small></div>` : ''}
    </div>` : ''}`;
}
...
// ═══════════════════════════════════════════════════════════════════════════════
// AUTO-REFRESH ENGINE — zero-friction provoz pro daily intraday
// ═══════════════════════════════════════════════════════════════════════════════
let autoRefreshTimer = null;
let autoRefreshOn = localStorage.getItem('AT_AUTO_REFRESH') === '1';

// Aktivní okno: 13:00–22:30 CZ (premarket přes US close)
function isActiveWindow() {
  const h = czHour(), m = czMinute();
  const mins = h * 60 + m;
  return mins >= 13 * 60 && mins <= 22 * 60 + 30;
}
...
function setAutoRefresh(on) {
  autoRefreshOn = on;
  localStorage.setItem('AT_AUTO_REFRESH', on ? '1' : '0');
  if (autoRefreshTimer) { clearInterval(autoRefreshTimer); autoRefreshTimer = null; }
  const btn = document.getElementById('autoBtn');
  if (on) {
    autoRefreshTimer = setInterval(doAutoRefresh, 60000); // každých 60s
    if (btn) { btn.textContent = '⏸ Auto: ON'; btn.classList.add('btn-primary'); }
    doAutoRefresh(); // hned první refresh
  } else {
    if (btn) { btn.textContent = '▶ Auto: OFF'; btn.classList.remove('btn-primary'); }
  }
}

// Auto-baseline: nastav baseline přesně ve 14:00 CZ pokud ještě není
function checkAutoBaseline() {
  const h = czHour(), m = czMinute();
  const baseAll = storage.baselines[todayKey()];
  // Mezi 14:00 a 14:05 CZ, pokud baseline ještě není dnešní → nastav
  if (h === 14 && m <= 5 && !baseAll) {
    saveBaselineFromCurrent();
    recordHistorySnapshot('auto-baseline');
  }
}

document.getElementById('autoBtn')?.addEventListener('click', () => setAutoRefresh(!autoRefreshOn));

// Init auto-refresh stav z localStorage
if (autoRefreshOn) setAutoRefresh(true);
else { const b = document.getElementById('autoBtn'); if (b) b.textContent = '▶ Auto: OFF'; }

// Auto-baseline check každou minutu
setInterval(checkAutoBaseline, 60000);
